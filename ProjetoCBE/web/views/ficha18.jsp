...
<%
    ...
    List<Ficha18> listaFichas;

    // Carrega as opções disponíveis
    List<Integer> anosDisponiveis = ficha18Controller.getAnosExistentes();
    List<Integer> trimestresDisponiveis = ficha18Controller.getTrimestresExistentes();
    ...
    if (temAno && temTrimestre) {
        ...
        listaFichas = ficha18Controller.getAllFichasByTrimestreAno(trim, ano);

    } else if (temAno) {
        // CASO 2: Tem APENAS Ano
        int ano = Integer.parseInt(anoFiltro);
        listaFichas = ficha18Controller.getAllFichasByAno(ano);

    } else if (temTrimestre) {
        // CASO 3: Tem APENAS Trimestre
        int trim = Integer.parseInt(trimestreFiltro);
        listaFichas = ficha18Controller.getAllFichasByTrimestre(trim);

    } else {
        // CASO 4: Busca tudo
        listaFichas = ficha18Controller.getAllFichas();
    }
    ...
    <body>
        ...
        <div class="view-container">
                ...
                <div>
                    <c:choose>
                        <c:when test = "${comissao.startsWith('GER SOLUCOES')}">
                            <input type="button" class="btn btn-validar" value="Validar" data-ficha="ficha18" id="valida" title="É necessário ter o cargo de gerente para validar as informações">
                        </c:when>
                        <c:otherwise>
                             <input type="button" class="btn btn-validar" value="Validar" data-ficha="ficha18" id="valida" title="É necessário ter o cargo de gerente para validar as informações">
                        </c:otherwise>
                    ...
                    <a href="../forms/ficha18.jsp"><input type="button" value="Adicionar" class="btn"></a>
                </div>
            </div>
            ...
            <div class="filtro-container" style="width: 95%; margin: 0 auto 20px auto; background-color: #f4f4f4; padding: 15px; border-radius: 8px; border: 1px solid #ddd;">
                <form action="ficha18.jsp" method="GET" style="display: flex; gap: 15px; align-items: center;">
                    ...
                    <% if ((anoFiltro != null && !anoFiltro.equals("todos")) || (trimestreFiltro != null && !trimestreFiltro.equals("todos"))) { %>
                        <a href="ficha18.jsp" style="text-decoration: none; color: #b5131d; font-weight: bold; margin-left: 10px;">Limpar Filtro</a>
                    <% } %>
                </form>
            </div>
                    ...
                    <tr>
                        ...
                        <td class="opcoes-col">
                            ...
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <a href="../edits/ficha18.jsp?id=${ficha.getId()}" class="option-btn" title="Editar">
                                        <img class="option-btn-img" src="../resources/imgs/editar.png" alt="editar"/>
                                    </a>
                                    <button class="option-btn delete" title="Excluir" data-id="${ficha.getId()}" data-ficha="ficha18">
                                        <img class="option-btn-img" src="../resources/imgs/lixo.png" alt="alt"/>
                                    </button>
                                </c:otherwise>
                            </c:choose>
