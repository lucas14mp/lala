...
//                resp.getWriter().write("{\"redirectUrl\": \"/ProjetoCBE/views/ficha11.jsp\"}");
    ...
    // Método novo com os tipos corrigidos (Ficha11Controle e int)
    public List<Ficha11Controle> getFichasComControladoraValida(int trimestre, int ano) {
        List<Ficha11Controle> todas = getAllFichasByTrimestreAno(trimestre, ano);
        List<Ficha11Controle> validas = new ArrayList<>();
        
        if (todas != null) {
            for (Ficha11Controle f : todas) {
                try {
                    // Tenta acessar a controladora. Se lançar exceção, o catch ignora o registro quebrado.
                    if (f != null && f.getFicha11Controladora() != null) {
                        validas.add(f);
                    }
                } catch (Exception e) {
                    System.out.println("Aviso: Ficha11Controle ID " + f.getId() + " possui controladora inválida. Ignorando no JSP.");
                }
            }
        }
        return validas;
