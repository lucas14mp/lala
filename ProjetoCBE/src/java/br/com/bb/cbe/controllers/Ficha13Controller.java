...
import br.com.bb.cbe.DAO.Ficha13DAO;
...
public class Ficha13Controller extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            if (tipoRequisicao.equals("post") || tipoRequisicao.equals("edit")) {
                ...
                ficha.setPrazoEmprestimo(req.getParameter("resposta-prazo"));
            ...
            switch (tipoRequisicao) {
                case "delete":
                    ...
                    Ficha13DAO.delete(id);
                    break;
                case "post":
                    ficha.setDataCriacao(new Date());
                    Ficha13DAO.create(ficha);
                    break;
                case "edit":
                    ficha.setDataCriacao(new Date());
                    ficha.setId(Integer.parseInt(req.getParameter("id")));
                    Ficha13DAO.update(ficha);
                    break;
                case "validacao":
                    ...
                    Ficha13DAO.validarFormularios(idsValidadosList, chaveFuncionario);
                    break;
                case "validacaoBatch":
                    List<String> arrayIdsValidados = processarValidacao(list);
                    Ficha13DAO.validarFormularios(arrayIdsValidados, chaveFuncionario);
            ...
            if (tipoRequisicao.equals("createbatch") || tipoRequisicao.equals("validacaoBatch")) {
                    ...
                    resp.getWriter().write("{\"redirectUrl\": \"/ProjetoCBE/views/ficha13.jsp\"}");
                    return;
            }
            resp.sendRedirect("views/ficha13.jsp");
        } catch (NumberFormatException e) {
            e.printStackTrace();
            req.setAttribute("mensagemErro", "O valor foi inserido em um formato inválido.");
            req.setAttribute("linkPaginaAnterior", "/ProjetoCBE/forms/ficha13.jsp");
            RequestDispatcher dispatcher = req.getRequestDispatcher("/errors/customError.jsp");
            dispatcher.forward(req, resp);
        }
    }

    public List<Ficha13> getAllFichas() {
        return Ficha13DAO.getAllFichas();
    }
    ...
    public List<Ficha13> getAllFichasByTrimestreAno(int trimestre, int ano) {
        return Ficha13DAO.getAllFichasByTrimestreAno(trimestre, ano);
    }

    public List<Ficha13> getAllFichasByTrimestre(int trimestre) {
        ...
        return Ficha13DAO.getAllFichasByTrimestre(trimestreParaDAO);
    }

    public List<Ficha13> getAllFichasByAno(int ano) {
        return Ficha13DAO.getAllFichasByAno(ano);
    }

    public List<Integer> getAnosExistentes() {
        return Ficha13DAO.getAnosExistentes();
    }

    public List<Integer> getTrimestresExistentes() {
        return Ficha13DAO.getTrimestresExistentes();
    }

    public Ficha13 getFichaById(int id) {
        Optional<Ficha13> optFicha13 = Ficha13DAO.getFichaById(id);
        if (optFicha13.isPresent()) {
            return optFicha13.get();
        }
        return null;
    }
    ...
    public static Justificativa processarJustificativa(List<Map<String, Object>> list, String chaveFuncionario){
        ...
        Gson gson = new Gson(); 
        List<Ficha13> fichas = Ficha13DAO.getAllFichas();
        String ficha13Json = gson.toJson(fichas);
        return ficha13Json;
}
          
        public List<Ficha13> getFichasPorPeriodo(int ano, int trimestre) {
        return Ficha13DAO.getAllFichasByTrimestreAno(trimestre, ano);
      }
